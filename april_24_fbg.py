...
images1 = numpy.memmap('tmp.npy', dtype = 'float32', mode = 'w+', shape = (2 * numImages, 96, 96, 3))
...
for i, code in enumerate(classes):
    ...
    for im in images:
        im2 = im[80 - 48 : 80 + 48, 80 - 48 : 80 + 48, :4]
        mask = (im2[:, :, 3] > 254.5)
        for r in range(rpts):
            background = backgrounds[numpy.random.randint(0, len(backgrounds))]

            ii = numpy.random.randint(0, background.shape[0] - 96)
            jj = numpy.random.randint(0, background.shape[1] - 96)

            backgroundSample = background[ii : ii + 96, jj : jj + 96]

            im3 = numpy.array(im2)
            #im3[:, :, 3] = (1.0 - mask) * 255.0
            im3[:, :, 0] += mask * backgroundSample[:, :, 0]
            im3[:, :, 1] += mask * backgroundSample[:, :, 1]
            im3[:, :, 2] += mask * backgroundSample[:, :, 2]
            im4 = im3[:, :, :3] / 255.0
            images1[i, :, :, :] = im4
            images1[i + numImages, :, :, :] = backgroundSample / 255.0
            idx += 1
...
images = numpy.memmap('tmp.npy', dtype = 'float32', mode = 'r', shape = (2 * numImages, 96, 96, 3))
...
x_image = tf.placeholder(tf.float32, shape=[None, 96, 96, 3])
...
pool = tf.nn.max_pool(h_conv5, ksize=[1, 6, 6, 1], strides=[1, 6, 6, 1], padding='SAME')
